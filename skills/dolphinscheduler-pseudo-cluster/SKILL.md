...
多集群使用 `--cluster <name>` 指定本次命令，或用 `ds-cli config cluster activate <name>` 设置默认集群。`ds-cli config show` 只展示当前生效集群、API 地址、字段来源、认证方式和 `has_token` 等布尔值，不输出 token、sessionId 或密码明文。
